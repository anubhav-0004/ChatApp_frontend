...
const Table = ({ rows, columns, heading, rowsPerPage = 5 }) => {
  ...
  return (
    <div className="max-h-[90vh] bg-[#564c4c] p-3 rounded-lg overflow-hidden">
      <h1 className="text-center m-2 font-semibold text-2xl text-[#180d3c] uppercase">{heading}</h1>

      <div className="overflow-x-auto max-md:max-h-[25.5rem] max-h-[30rem]">
        <table {...getTableProps()} className="table-auto border-collapse border border-gray-300 w-full">
          <thead>
            {headerGroups.map((headerGroup) => (
              <tr {...headerGroup.getHeaderGroupProps()} className="bg-[#895f45d1]">
          ...
          <tbody {...getTableBodyProps()}>
            {tableRows.map((row) => {
              prepareRow(row);
              return (
                <tr {...row.getRowProps()} className="even:bg-gray-300 odd:bg-[#6a6161] hover:even:bg-gray-200 hover:odd:bg-[#9a8383]">
                  {row.cells.map((cell) => (
                    <td
                      {...cell.getCellProps()}
