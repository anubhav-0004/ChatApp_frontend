...
const StyledPic = ({setAvatar}) => {
  ...
  return (
    <div className='w-fit h-fit my-auto mx-auto flex justify-center items-end gap-0'>
        ...
        <input type="file" name="avatar" id="inpFile" accept='image/*' className='hidden' onChange={handleImageChange}/>
        <label htmlFor="inpFile" className='bg-transparent mx-auto cursor-pointer'><IoCamera className='min-w-8 min-h-8 max-md:min-w-6 max-md:min-h-6 mx-auto text-zinc-700 bg-gray-300 p-1 border rounded-[50%]'/></label>
    </div>
  )
}
